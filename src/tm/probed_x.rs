...
//! [`ThreadMapX`](https://docs.rs/thread_map/latest/thread_map/) instance and aggregates the values.
...
//! ```rust
#![doc = include_str!("../../examples/tm_probed_x_i32_accumulator.rs")]
//! ````

//!
//! ## Other examples
//!
//! See another example at [`examples/tm_probed_x_map_accumulator`](https://github.com/pvillela/rust-thread-local-collect/blob/main/examples/tm_probed_x_map_accumulator.rs).
...
/// - A [`ThreadMapX`] object that holds the thread-local values and a function to initialized those values.
...
impl<T, U> Control<T, U> {
    ...
    pub fn new(
        ...
        let tmap = ThreadMapX::new(value_init);
        Control {
            tmap: tmap.into(),
    ...
    ///
    /// # Panics
    /// - If `self`'s [`ThreadMapX`] state is poisoned.
    /// - If `self`'s accumulated value mutex is poisoned.
