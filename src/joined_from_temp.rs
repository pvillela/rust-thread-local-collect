...
use crate::common::{ControlBase, ControlInner, HolderBase};
use std::{
    ...
    U: 'static,
{
    ...
    pub fn with_acc<V>(&self, lock: &mut MutexGuard<'_, U>, f: impl FnOnce(&U) -> V) -> V {
        ControlBase::with_acc(self, lock, f)
    }
    ...
    pub fn take_acc(&self, lock: &mut MutexGuard<'_, U>, replacement: U) -> U {
        ControlBase::take_acc(self, lock, replacement)
    }
}
